...
static bool parse_notify(struct pool *pool, json_t *val)
{
	...
	doncoinbase1 = json_array_string(val, 4);
	doncoinbase2 = json_array_string(val, 5);
	bbversion = json_array_string(val, 7);
	nbit = json_array_string(val, 8);
	ntime = json_array_string(val, 9);
	clean = json_is_true(json_array_get(val, 10));

	if (!job_id || !prev_hash || !coinbase1 || !coinbase2 || !doncoinbase1 || !doncoinbase2 || !bbversion || !nbit || !ntime) {
	...
	doncb1_len = strlen(doncoinbase1) / 2;
        doncb2_len = strlen(doncoinbase2) / 2;
	pool->swork.bbversion = bbversion;
	pool->swork.nbit = nbit;
	pool->swork.ntime = ntime;
	pool->swork.clean = clean;
	alloc_len = pool->swork.cb_len = cb1_len + pool->n1_len + pool->n2size + cb2_len;
	donalloc_len = pool->swork.doncb_len = doncb1_len + pool->n1_len + pool->n2size + doncb2_len;
	...
	doncb1 = (unsigned char *)calloc(doncb1_len, 1);
	if (unlikely(!cb1))
		quithere(1, "Failed to calloc cb1 in parse_notify");
	hex2bin(cb1, coinbase1, cb1_len);
	if (unlikely(!doncb1))
		quithere(1, "Failed to calloc doncb1 in parse_notify");
	hex2bin(doncb1, doncoinbase1, doncb1_len);

	cb2 = (unsigned char *)calloc(cb2_len, 1);
	doncb2 = (unsigned char *)calloc(doncb2_len, 1);
	if (unlikely(!cb2))
		quithere(1, "Failed to calloc cb2 in parse_notify");
	hex2bin(cb2, coinbase2, cb2_len);
	if (unlikely(!doncb2))
		quithere(1, "Failed to calloc doncb2 in parse_notify");
	hex2bin(doncb2, doncoinbase2, doncb2_len);
	free(pool->coinbase);
	align_len(&alloc_len);
	pool->coinbase = (unsigned char *)calloc(alloc_len, 1);
	if (unlikely(!pool->coinbase))
		quit(1, "Failed to calloc pool coinbase in parse_notify");
	pool->doncoinbase = (unsigned char *)calloc(donalloc_len, 1);
	if (unlikely(!pool->doncoinbase))
		quit(1, "Failed to calloc pool doncoinbase in parse_notify");
	memcpy(pool->coinbase, cb1, cb1_len);
	memcpy(pool->coinbase + cb1_len, pool->nonce1bin, pool->n1_len);
	memcpy(pool->doncoinbase, doncb1, doncb1_len);
	memcpy(pool->doncoinbase + doncb1_len, pool->nonce1bin, pool->n1_len);
	memcpy(pool->coinbase + cb1_len + pool->n1_len + pool->n2size, cb2, cb2_len);
	memcpy(pool->doncoinbase + doncb1_len + pool->n1_len + pool->n2size, doncb2, doncb2_len);
	...
	if (opt_protocol) {
		...
		applog(LOG_DEBUG, "doncoinbase1: %s", doncoinbase1);
		applog(LOG_DEBUG, "doncoinbase2: %s", doncoinbase2);
