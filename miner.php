...
# Set $readonly to false then it will check Triglav 'privileged'
$readonly = false;
#
...
$poolinputs = false;
#
# Set $rigs to an array of your Triglav rigs that are running
...
$mcastexpect = 0;
#
# API Multicast address all Triglav miners are listening on
$mcastaddr = '224.0.0.75';
#
# API Multicast UDP port all Triglav miners are listening on
$mcastport = 4028;
#
# The code all Triglav miners expect in the Multicast message sent
$mcastcode = 'FTW';
#
# UDP port Triglav miners are to reply on (by request)
$mcastlistport = 4027;
#
...
function mcastrigs()
{
 ...
 do
 {
	...
	$buf = "triglav-$mcastcode-$mcastlistport";
...
{
 $dthead = array($headname => 1, 'STATUS' => 1, 'Description' => 1, 'When' => 1, 'API' => 1, 'triglav' => 1);
 showhead('', $dthead);
...
function pagebuttons($rig, $pg)
{
 ...
 if ($rig !== null && $readonly === false)
 {
	...
	if (count($rigs) > 1)
		$rg = " Rig $rig";
	echo "<input type=button value='Restart' onclick='prc(\"restart&rig=$rig\",\"Restart Triglav$rg\")'>";
	echo "&nbsp;<input type=button value='Quit' onclick='prc(\"quit&rig=$rig\",\"Quit Triglav$rg\")'>";
 }
 refreshbuttons();
