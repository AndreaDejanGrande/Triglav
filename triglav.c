...
static unsigned char *__gbt_merkleroot(struct pool *pool)
{
	...
	int i, txns;
        ...
        //gbt_txns + 2 coinbase txs + 1 hash duplicate
	...
	if (pool->gbt_txns)
		//32 for 1 coinbase tx; 64 for 2 coinbase txs
		memcpy(merkle_hash + 64, pool->txn_hashes, pool->gbt_txns * 32);

        //gbt_txns + 2 coinbase txs
	txns = pool->gbt_txns + 2;
	while (txns > 1) {
		if (txns % 2) {
...
static void gen_gbt_work(struct pool *pool, struct work *work)
{
	...
	memcpy(pool->doncoinbase + pool->nonce2_offset, &nonce2le, pool->n2size);
	pool->nonce2++;
	...
	work->doncoinbase = bin2hex(pool->doncoinbase, pool->doncoinbase_len);
...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
	unsigned char merkle_root[32], cb_hash[32], doncb_hash[32], merkle_sha[64];
	...
	gen_hash(pool->coinbase, cb_hash, pool->swork.cb_len);
	gen_hash(pool->doncoinbase, doncb_hash, pool->swork.cb_len);
        memcpy(merkle_sha, cb_hash, 32);
	memcpy(merkle_sha + 32, doncb_hash, 32);
